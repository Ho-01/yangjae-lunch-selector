...
일반·내 주변·점심방 결과 공유는 모두 브라우저에서 PNG 카드로 생성됩니다.
파일 공유를 지원하는 모바일 브라우저에서는 공유창을 열고, 그 외 환경에서는
같은 이미지를 다운로드합니다.

로그인 없이 만든 점심방은 현재 브라우저의 `최근 참여한 방`에 최대 10개 저장됩니다.
방 코드와 구성원 토큰을 서버에서 검증해 기존 닉네임과 권한으로 재입장하며, 다른
브라우저·기기와는 동기화되지 않습니다.

기본 UI는 shadcn/ui 공통 컴포넌트를 사용합니다. `components.json`이 생성 설정을,
`src/components/ui/`가 프로젝트에서 관리하는 버튼·입력·선택·체크·스위치 등의
컴포넌트 소스를 담습니다.
