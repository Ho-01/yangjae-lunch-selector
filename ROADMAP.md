...
- [x] Standardize the current public product name as `점심룰렛`.
...
- Rebrand the public product from `점심룰렛` to `식사가챠` across the app shell,
...
- Support a randomized result-message pool while keeping the selected menu prominent
  ...
  - `💀 다시 돌리기? 그건 비겁한 선택입니다.`
  - `🔥 오늘의 점심은 이미 정해져 있었다.`
- Define message variants that work with and without a menu-name placeholder, avoid
