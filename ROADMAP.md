...
- [x] 새로고침하거나 브라우저를 다시 열어도 기존 구성원 세션으로 방 다시 들어가기
- [x] 활성·최근 방을 확인하는 `최근 참여한 방` 목록
  - 방 이름 또는 코드, 상태, 인원, 최근 활동, 만료 시간 표시
  - 각 항목에 명확한 `다시 들어가기` 버튼 제공
- [x] 기존 단일 localStorage 세션을 최근 방 최대 10개 구조로 자동 변환
- [x] 서버에서 해시된 토큰을 검증한 뒤 기존 구성원·닉네임·방장 상태 복원
- [x] 만료되거나 유효하지 않은 방의 이유 표시와 목록 삭제
- [ ] 방 만료 시간을 제품 사용 데이터에 따라 6시간에서 조정할지 검토
- [x] 방장 위임과 현재 사용자·방장 구성원 식별 개선
- [ ] 좋아요와 거부권이 최종 후보에 미친 영향 설명

## 완료 — M4 점심방 채팅 (`0.5.0`)

- [x] 방을 만들거나 참여하면 사용할 수 있는 실시간 채팅
- [x] 보낸 사람, 전송 시각, 로딩·빈 상태·오류 상태 표시
- [x] 300자 제한, 초당 전송 제한, 방 삭제 시 메시지 자동 삭제
- [x] 채팅과 식당 검색·후보 직접 추가 UI를 명확히 분리
- [x] 토큰 검증 RPC, 최근 100개 조회와 테이블 직접 접근 차단
- [ ] 실제 사용량을 바탕으로 신고·차단 기능 필요성 검토
...
## 완료 — M6 룰렛 긴장감 연출 (`0.6.0`)

- [x] 당첨 항목 중앙이 아닌 좌우 경계 안쪽의 무작위 지점에 정지
- [x] 포인터는 움직이지 않고 항상 고정
- [x] 바퀴는 역회전 없이 한 방향으로 자연스럽게 감속
- [x] 양쪽 4% 안전 여백을 제외한 항목 내부 전체에 균등 랜덤 정지
- [x] 마지막 구간에서도 이동이 보이는 긴 감속 곡선과 5.4초 회전 시간
- [x] 최종 포인터와 실제 당첨 결과가 항상 일치하도록 안전 여백 보장
- [x] 점심방에서는 같은 당첨 결과와 시작 시각으로 동일한 정지 지점 재현
- [x] `prefers-reduced-motion` 환경에서는 경계 연출 없이 짧고 안정적으로 정지
- [x] 작은 가중치 세그먼트에서도 경계 밖에 최종 정지하지 않는 수학 테스트

## 완료 — M7 룰렛 피드백 (`0.7.0`)

- [x] 포인터가 세그먼트 경계를 통과할 때 회전 속도에 맞춘 틱 효과음
- [x] 당첨 시 짧은 완료음과 색종이 폭죽 효과
- [x] 외부 음원 파일 없이 Web Audio로 효과음 생성
- [x] 브라우저 자동재생 정책과 `prefers-reduced-motion` 접근성 처리
